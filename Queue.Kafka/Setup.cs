...
{
    public static void ConfigureKafka(this IServiceCollection services, string bootstrapServers, bool activateConsumers = true)
    {
        ...
        {
            var queueType = typeof(KafkaMessageQueue<>).MakeGenericType(messageType);
            ...
            services.AddSingleton(typeof(IMessageQueue<>).MakeGenericType(messageType), provider =>
                ActivatorUtilities.CreateInstance(provider, queueType, bootstrapServers, messageType.Name,
